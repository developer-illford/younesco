(function ($) {
  ...
  $(document).ready(function () {
    $('.projects-three__wrapper .projects-three__item').on("mousemove", function () {
      $('.projects-three__wrapper .projects-three__item').removeClass("active");
      $(this).addClass("active");
    });
  });

  // distance slider
  let slider = $("#distance-slider");
  let handle = $(".distance-slider-handle");
  let bar = $(".distance-slider-bar");
  let output = $("#distance-slider-value");

  let min = parseInt(slider.attr("data-min"));
  let max = parseInt(slider.attr("data-max"));
  let step = parseInt(slider.attr("data-step"));
  let defaultValue = parseInt(slider.attr("data-default"));
  ...
  function setPositionFromValue(value) {
    ...
    if (isRTL()) {
      handle.css("right", position + "px");
      handle.css("left", "auto");
      bar.css("width", position + "px");
    } else {
  ...
  function getValueFromPosition(position) {
    let sliderWidth = getSliderWidth();
    let percent = position / sliderWidth;
  ...
  function handleMove(event) {
    let sliderWidth = getSliderWidth();
    let position = event.pageX - slider.offset().left;
    ...
    setPositionFromValue(newValue);
  }

  setPositionFromValue(defaultValue);

  $(window).on("resize", function () {
    setPositionFromValue(parseInt(output.text()));
  });
  ...
  //accordion
  if ($(".transwide-accordion").length) {
    var accordionGrp = $(".transwide-accordion");
        ...
        $(this)
          ...
          .on("click", function () {
            if ($(this).parent().hasClass("active") === false) {
              $(".transwide-accordion." + accordionName)
                .find(".accordion")
                .removeClass("active");
              $(".transwide-accordion." + accordionName)
                .find(".accordion")
  ...
  function thmOwlInit() {
    // owl slider
    let transwideowlCarousel = $(".transwide-owl__carousel");
    if (transwideowlCarousel.length) {
      transwideowlCarousel.each(function () {
        let elm = $(this);
    ...
    let transwideowlCarouselNav = $(
      ".transwide-owl__carousel--custom-nav"
    );
    if (transwideowlCarouselNav.length) {
      transwideowlCarouselNav.each(function () {
        let elm = $(this);
    ...
    let transwideowlCarouselCustomDots = $(".transwide-owl__carousel--custom-dots");
    if (transwideowlCarouselCustomDots.length) {
      transwideowlCarouselCustomDots.each(function () {
        let elm = $(this);
        let transwideowlCarouselThumb = elm.data("thumb-elm");
        $(transwideowlCarouselThumb).each(function () {
          let self = $(this);
  ...
  //Strech Column
  function transwide_stretch() {
    var i = $(window).width();
    $(".row .transwide-stretch-element-inside-column").each(function () {
      var $this = $(this),
